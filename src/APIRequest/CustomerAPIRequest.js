...
import { SetCustomerList, SetCustomerListTotal,OnChangeCustomerInput, ResetFormValue } from "../redux/state-slice/customer-slice";
...
// for updating purpose. using objectId get the customer's details and fill
export async function FillCustomerFormRequest(ObjectID) {
    try {
        ...
        if (result.status === 200 && result.data['status'] === "success") {
            ...
            return true;
            /*
            const desiredFields = ['CustomerName', 'Phone', 'Email', 'Address'];

            if (result.status === 200 && result.data.status === "success") {
            const formValue = result.data.data[0];
            const inputChanges = desiredFields.map(field => ({
            Name: field,
            Value: formValue[field] || ''
             }));
            inputChanges.forEach(inputChange => {
            store.dispatch(OnChangeCustomerInput(inputChange));
            });
            return true;
            }
...
// delete customer from database in CustomerList page
